  ...
  for(dir=XUP;dir<=TUP;dir++){
    ...
    for(term=0;term<nterms;term++){
      ...
      FORALLFIELDSITES(i){
        ...
        // multiply by alpha_l in rational function expansion
        scalar_mult_add_su3_matrix( &(oprod_along_path[0][i]), &tmat, 
				    residues[term]*one_hop_coeff, &(oprod_along_path[0][i]) );
      }
      cleanup_gather(mtag[k]);
      k=1-k; // swap 0 and 1
    } /* end loop over terms in rational function expansion */
  ...
  for(dir=XUP;dir<=TUP;dir++){ //AB loop on directions, path table is not needed
    ...
    for(term=0;term<nterms;term++){
      ...
      FORALLFIELDSITES(i){
        ...
        // multiply by alpha_l in rational function expansion
        scalar_mult_add_su3_matrix( &(oprod_along_path[0][i]), &tmat, 
				    residues[term]*three_hop_coeff, &(oprod_along_path[0][i]) );
      }
      cleanup_gather(mtag[k]);
      k=1-k; // swap 0 and 1
    } /* end loop over terms in rational function expansion */
...
static void new_outer_product_append( Real* one_hop_coeff, Real* three_hop_coeff, 
				  su3_vector **multi_x, int nterms,
				  su3_matrix *one_hop_oprod[4], 
				  su3_matrix *three_hop_oprod[4] )
{
  ...
  for(dir=XUP;dir<=TUP;dir++){
    ...
    for(term=0;term<nterms;term++){
      ...
      FORALLFIELDSITES(i){
        ...
        // multiply by alpha_l in rational function expansion
        scalar_mult_add_su3_matrix( &(oprod_along_path[0][i]), &tmat, 
				  one_hop_coeff[term], &(oprod_along_path[0][i]) );
      }
      cleanup_gather(mtag[k]);
      k=1-k; // swap 0 and 1
    } /* end loop over terms in rational function expansion */
    
     // link_gather_connection_hisq is crucial  
    // It shifts the fields forwards again. 
  ...
  for(dir=XUP;dir<=TUP;dir++){ //AB loop on directions, path table is not needed
    ...
    for(term=0;term<nterms;term++){
      ...
      FORALLFIELDSITES(i){
        ...
        // multiply by alpha_l in rational function expansion
        scalar_mult_add_su3_matrix( &(oprod_along_path[0][i]), &tmat, 
				  three_hop_coeff[term], &(oprod_along_path[0][i]) );
      }
      cleanup_gather(mtag[k]);
      k=1-k; // swap 0 and 1
    } /* end loop over terms in rational function expansion */
   
    // Shift back in the positive direction 
    // - very wasteful!! 
    link_gather_connection_hisq( oprod_along_path[0], oprod_along_path[1], 
				 mat_tmp0, DIR3(dir) );
					   ...
					   fermion_links_t *fl)
{
  ...
  for(dir=XUP; dir<=TUP; ++dir){
    ...
    memset(one_link_oprod[dir], 0, sites_on_node*sizeof(su3_matrix));
    memset(three_link_oprod[dir], 0, sites_on_node*sizeof(su3_matrix));
    memset(staple_oprod[dir], 0, sites_on_node*sizeof(su3_matrix));
  }
  
  n_naik_shift = 0;
  n_orders_naik_current = n_order_naik_total;
  n_orders_naik_current = n_order_naik_total;
  Real one_hop_coeff =   ap->p2.act_path_coeff.one_link*2.0*eps;
  Real three_hop_coeff = ap->p2.act_path_coeff.naik*2.0*eps;
  double* one_hop_res = malloc(n_order_naik_total*sizeof(double));
  double* three_hop_res = malloc(n_order_naik_total*sizeof(double));
  for(i=0; i<n_order_naik_total; ++i){
    one_hop_res[i] = one_hop_coeff*residues[i];
    three_hop_res[i] = three_hop_coeff*residues[i]; 
  }

  QudaOprodArgs_t oprod_args; 
  oprod_args.num_terms = n_order_naik_total; 
  oprod_args.one_hop_coeff = one_hop_res; 
  oprod_args.three_hop_coeff = three_hop_res;
  oprod_args.quark_fields = multi_x;


 
  n_naik_shift = n_orders_naik[0];
  int num_terms, term, index;
  num_terms = 0; index=0;
  for(inaik=1; inaik<n_naiks; ++inaik){  num_terms += n_orders_naik[inaik]; }
  double* one_hop_residues = malloc(num_terms*sizeof(double));
  double* three_hop_residues = malloc(num_terms*sizeof(double));
  double* new_residues = malloc(num_terms*sizeof(double));
  for(inaik=1; inaik<n_naiks; ++inaik){
    ...
    for(term=0; term<n_orders_naik[inaik]; ++term){
     new_residues[term] = *(residues+n_naik_shift+term);
     one_hop_residues[index] = one_hop_coeff*(*(residues+n_naik_shift+term));
     three_hop_residues[index] = three_hop_coeff*(*(residues+n_naik_shift+term));
      ++index;
    }
    n_naik_shift += n_orders_naik[inaik];  
  } // end loop over inaik
  ...
  oprod_args2.num_terms = num_terms; 
  oprod_args2.one_hop_coeff = one_hop_residues; 
  oprod_args2.three_hop_coeff = three_hop_residues; 
  oprod_args2.quark_fields   =  multi_x + n_orders_naik[0];
  ...
  level2_coeff[2] /= level2_coeff[0];
  level2_coeff[3] /= level2_coeff[0];
  level2_coeff[4] /= level2_coeff[0];
  level2_coeff[5] /= level2_coeff[0];
  double fat7_coeff[6];
  ...
  qudaComputeOuterProductExternal2(PRECISION, 
				   oprod_args, 
				   oprod_args2,
				   staple_oprod,
				   one_link_oprod,
 ...
 for(dir=0; dir<=TUP; ++dir){
    for(i=0; i<18*sites_on_node; ++i){
     node0_printf("staple_oprod[%d][0][%d] = %lf\n", dir, i, *((Real*)staple_oprod[dir] + i));
     node0_printf("one_link_oprod[%d][0][%d] = %lf\n", dir, i, *((Real*)one_link_oprod[dir] + i));
     node0_printf("three_link_oprod[%d][0][%d] = %lf\n", dir, i, *((Real*)three_link_oprod[dir] + i));
    }
  }
...
/*
  qudaHisqForceNewer(PRECISION, oprod_args, oprod_args2, 
		   level2_coeff, fat7_coeff, 
		   W_unitlink, V_link, U_link, momentum);
*/
  ...
  free(three_hop_res);
  free(new_residues);
  free(one_hop_residues);
  free(three_hop_residues);
