...
static void qudaComputeOuterProductInternal(QudaPrecision precision, 
				            double one_hop_coeff[],
				            double three_hop_coeff[],
			                    int num_terms,
				            void** quark_fields, 
					    cudaGaugeField* const cudaOprod,
					    cudaGaugeField* const cudaLongLinkOprod)
{
  ...
  qudaGaugeParam.cpu_prec  = precision;
  qudaGaugeParam.cuda_prec = precision;
  ...
  quarkParam.x[0] /= 2; // Half field
  quarkParam.precision = precision;
  quarkParam.pad = 0;
  quarkParam.siteSubset = QUDA_PARITY_SITE_SUBSET;
  ...
  quarkParam.create     = QUDA_ZERO_FIELD_CREATE;
  quarkParam.fieldLocation = QUDA_CUDA_FIELD_LOCATION;
  cudaColorSpinorField* cudaEvenQuarkField = new cudaColorSpinorField(quarkParam);
  cudaColorSpinorField* cudaOddQuarkField = new cudaColorSpinorField(quarkParam);
#define QUDA_VER ((10000*QUDA_VERSION_MAJOR) + (100*QUDA_VERSION_MINOR) + QUDA_VERSION_SUBMINOR)
#if (QUDA_VER > 400)
  initLatticeConstants(*cudaOprod);
  initGaugeConstants(*cudaOprod);
#else
  initGaugeFieldConstants(*cudaOprod);
#endif

  quarkParam.fieldLocation = QUDA_CPU_FIELD_LOCATION;
  quarkParam.create = QUDA_REFERENCE_FIELD_CREATE;

  printfQuda("Calling computeOuterProductInternal\n");
  createOprodEvents();
  
  const int half_volume = getVolume(quarkParam.x); // division by 2 included 

  printfQuda("half_volume = %d\n", half_volume);
  
  for(int i=0; i<num_terms; ++i){
    quarkParam.v = quark_fields[i];
    cpuColorSpinorField* cpuEvenQuarkField = new cpuColorSpinorField(quarkParam);

    quarkParam.v = ((char*)quark_fields[i] +  half_volume*6*precision); 
    cpuColorSpinorField* cpuOddQuarkField = new cpuColorSpinorField(quarkParam);
 
    *cudaEvenQuarkField = *cpuEvenQuarkField;
    *cudaOddQuarkField = *cpuOddQuarkField;

    float* even_field1 = (float*)malloc(cudaEvenQuarkField->Bytes());
    float* odd_field1  = (float*)malloc(cudaOddQuarkField->Bytes());
    cudaMemcpy(even_field1, cudaEvenQuarkField->V(), cudaEvenQuarkField->Bytes(), cudaMemcpyDeviceToHost);
    cudaMemcpy(odd_field1, cudaOddQuarkField->V(), cudaEvenQuarkField->Bytes(), cudaMemcpyDeviceToHost);


    computeOuterProdCuda(0, 0, qudaGaugeParam, one_hop_coeff[i], *cudaEvenQuarkField, *cudaOddQuarkField, cudaOprod);

    float* even_field2 = (float*)malloc(cudaEvenQuarkField->Bytes());
    float* odd_field2  = (float*)malloc(cudaOddQuarkField->Bytes());
    cudaMemcpy(even_field2, cudaEvenQuarkField->V(), cudaEvenQuarkField->Bytes(), cudaMemcpyDeviceToHost);
    cudaMemcpy(odd_field2, cudaOddQuarkField->V(), cudaEvenQuarkField->Bytes(), cudaMemcpyDeviceToHost);

    for(int j=0; j<cudaEvenQuarkField->Bytes()/precision; ++j){
	    printfQuda("field[%d] = %lf,   %lf\n", j, odd_field1[j], odd_field2[j]);			
	    if(fabs(odd_field1[j] - odd_field2[j]) > 0.001){
	      printfQuda("Difference in field[%d] = %lf,   %lf\n", j, odd_field1[j], odd_field2[j]);			
	    }
    }

    computeOuterProdCuda(1, 0, qudaGaugeParam, one_hop_coeff[i], *cudaOddQuarkField, *cudaEvenQuarkField, cudaOprod);
    cudaThreadSynchronize();
    // long-link outer products
    computeOuterProdCuda(0, 1, qudaGaugeParam, three_hop_coeff[i], *cudaEvenQuarkField, *cudaOddQuarkField, cudaLongLinkOprod);
    cudaThreadSynchronize();
    computeOuterProdCuda(1, 1, qudaGaugeParam, three_hop_coeff[i], *cudaOddQuarkField, *cudaEvenQuarkField, cudaLongLinkOprod);
    cudaThreadSynchronize();
    ...
    delete cpuEvenQuarkField;
    delete cpuOddQuarkField;
  }

  destroyOprodEvents();
  if(cudaEvenQuarkField){ delete cudaEvenQuarkField; cudaEvenQuarkField=NULL; }
  if(cudaOddQuarkField){ delete cudaOddQuarkField; cudaOddQuarkField=NULL; }
...
void qudaComputeOuterProductExternal2(int precision, 
				     QudaOprodArgs_t oprod_args1,
				     QudaOprodArgs_t oprod_args2,
				     void* const staple_src[4],
			             ...
			             void* const three_link_src[4])
{
  ...
  printfQuda("Calling qudaComputeOuterProductExternal\n");
...
//  gParam.gauge = (void*)staple_src;
  cpuGaugeField  *cpuStapleOprod   = new cpuGaugeField(gParam);
//  gParam.gauge = (void*)one_link_src;
  cpuGaugeField  *cpuOneLinkOprod  = new cpuGaugeField(gParam);
//  gParam.gauge = (void*)three_link_src; 
  cpuGaugeField  *cpuLongLinkOprod = new cpuGaugeField(gParam);
  gParam.create = QUDA_ZERO_FIELD_CREATE;

  cudaGaugeField *cudaOprod = new cudaGaugeField(gParam);
  cudaGaugeField *cudaLongLinkOprod = new cudaGaugeField(gParam);


  qudaComputeOuterProductInternal(local_precision, 
				  oprod_args1.one_hop_coeff, 
				  oprod_args1.three_hop_coeff, 
				  oprod_args1.num_terms, 
				  oprod_args1.quark_fields, 
				  cudaOprod,
				  cudaLongLinkOprod);
  cudaThreadSynchronize();
  
  cudaOprod->saveCPUField(*cpuStapleOprod, QUDA_CPU_FIELD_LOCATION);


  cudaLongLinkOprod->saveCPUField(*cpuLongLinkOprod, QUDA_CPU_FIELD_LOCATION);

  for(int dir=0; dir<4; ++dir){
    memcpy(staple_src[dir],((void**)cpuStapleOprod->Gauge_p())[dir],cpuStapleOprod->Bytes()/4);
  }



  printfQuda("oprod_args2.num_terms = %d\n", oprod_args2.num_terms);
  for(int i=0; i<oprod_args2.num_terms; ++i){
    printfQuda("one_hop_coeff[%d] = %lf\n", i, oprod_args2.one_hop_coeff[i]); 
    printfQuda("three_hop_coeff[%d] = %lf\n", i, oprod_args2.three_hop_coeff[i]); 
  }
  qudaComputeOuterProductInternal(local_precision, 
				  oprod_args2.one_hop_coeff, 
				  oprod_args2.three_hop_coeff, 
				  oprod_args2.num_terms, 
				  oprod_args2.quark_fields, 
				  cudaOprod,
				  cudaLongLinkOprod);


  cudaOprod->saveCPUField(*cpuOneLinkOprod, QUDA_CPU_FIELD_LOCATION);
  cudaLongLinkOprod->saveCPUField(*cpuLongLinkOprod, QUDA_CPU_FIELD_LOCATION);



  // I should just be setting pointers here!
  for(int dir=0; dir<4; ++dir){
    memcpy(one_link_src[dir],((void**)cpuOneLinkOprod->Gauge_p())[dir],cpuOneLinkOprod->Bytes()/4);
    memcpy(three_link_src[dir],((void**)cpuLongLinkOprod->Gauge_p())[dir],cpuLongLinkOprod->Bytes()/4);
  }

  // Don't need to reorder fields, I don't think!
  if(cudaOprod){ delete cudaOprod; cudaOprod=NULL; }
  if(cudaLongLinkOprod){ delete cudaLongLinkOprod; cudaLongLinkOprod=NULL; }

  if(cpuStapleOprod){ delete cpuStapleOprod; cpuStapleOprod=NULL; }
  if(cpuOneLinkOprod){ delete cpuOneLinkOprod; cpuOneLinkOprod=NULL; }
  if(cpuLongLinkOprod){ delete cpuLongLinkOprod; cpuLongLinkOprod=NULL; }
