...
#ifdef INTERNAL_COMPUTE
if(x4 < X4 -3){
#endif
#ifdef BOUNDARY_COMPUTE
  if(x4 >=X4 -3 ){
#endif  
{
    //direction: +T
    ...
    // read spinor from device memory
#if defined(FULL_COMPUTE) || defined(BOUNDARY_COMPUTE)
    int nbr_idx1 = sp_idx_1st_nbr;
    int stride = sp_stride;
    ...
    READ_1ST_NBR_SPINOR(SPINORTEX, nbr_idx1, stride);    
    ...
    READ_3RD_NBR_SPINOR(SPINORTEX, nbr_idx3, stride); 
#endif
...
#if defined(INTERNAL_COMPUTE) || defined(BOUNDARY_COMPUTE)
  }
#endif
...
#ifdef DSLASH_XPAY
#if defined(FULL_COMPUTE) || defined(INTERNAL_COMPUTE)
READ_ACCUM(ACCUMTEX)
