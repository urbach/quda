// *** CUDA TWIST DSLASH DAGGER ***

//implementation of the following operators:
//1) dagger{R(a, b)} * dagger{Dslash},
//where
//
//dagger{R(a, b)} = a + i*b*gamma5, 
//a = 1 / (1 + (2*kappa*mu)^2), b = a*2*kappa*mu; 
//
//2) dagger{T(a)} + b*dagger{Dslash} 
//
//where
//dagger{T(a)} = 1 - i*a*gamma5,
...
#ifndef DSLASH_XPAY//set paramters for tm_dslashDagger{...}Kernel

  b = 2 * kappa * mu;
  a = 1 / (1 + b * b);
  b *= a;

#else//set parameters for tm_dslashXpayDagger{...}Kernel 
...
{
    // Projector dagger{R(a,b)} * P0+
    // a	b 	i*b	 i*a 
    // b 	a 	i*a 	i*b 
    // i*b 	-i*a 	a 	-b 
    // -i*a	-i*b 	-b 	a 
    
//Note: dslash dagger requires extra operations!
...
#define tmp3_im tmp3.y
    ...
    //apply (1 - i*a*gamma5) to the input spinor and then add to (b * output spinor)
    ...
    o01_re = b * o01_re + tmp0_re;//1
    o01_im = b * o01_im + tmp0_im;//1
    o11_re = b * o11_re + tmp1_re;//4
    o11_im = b * o11_im + tmp1_im;//4
    o21_re = b * o21_re + tmp2_re;//7
    o21_im = b * o21_im + tmp2_im;//7
    o31_re = b * o31_re + tmp3_re;//10
    o31_im = b * o31_im + tmp3_im;//10
