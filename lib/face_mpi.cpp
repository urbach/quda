...
void FaceBuffer::exchangeFacesWait(cudaColorSpinorField &out, int dagger, int dir)
{
  ...
  if(out.precision == 4){
    static int count=0;
    if(count < 10){
      printfQuda("SP: dir=%d, pack_time=%.2f, 1st_memcpy_time=%f, mpi_time=%.2f ms, 2nd memcpy_time=%.2f ms,  unpack_time=%.2f ms, total=%.2f ms\n", dir,
    ...
    count++;
  }else if(out.precision ==8){
    static int count=0;
    if(count < 10){
      printfQuda("DP: dir=%d, pack_time=%.2f, 1st_memcpy_time=%f, mpi_time=%.2f ms, 2nd memcpy_time=%.2f ms,  unpack_time=%.2f ms, total=%.2f ms\n", dir,
		 pack_time[0] + pack_time[1], 
		 first_memcpy_time[0] + first_memcpy_time[1],	     
		 mpi_time[0] + mpi_time[1],
		 second_memcpy_time[0] + second_memcpy_time[1],
