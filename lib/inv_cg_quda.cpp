		  ...
		  cudaColorSpinorField &y, QudaInvertParam *invert_param)
{
  ...
  if (k==invert_param->maxiter) 
    PRINTF("Warning: exceeded maximum iterations %d", invert_param->maxiter);

  if (invert_param->verbosity >= QUDA_SUMMARIZE)
    PRINTF("CG: Reliable updates = %d\n", rUpdate);
  ...
  if (invert_param->verbosity >= QUDA_SUMMARIZE){
    PRINTF("Converged after %d iterations, r2 = %e, relative true_r2 = %e\n", 
