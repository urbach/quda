...
const int maxReduce = 3;
cudaEvent_t sumCompletion;
...
static QudaSumFloat *hd_reduce=0; // this is the zero copy alias of h_reduce

double3 *dd_reduce=0; // device pointer used for DD reductions on the device only 
...
{  
  ...
  if (!d_reduce) {
    if (cudaMalloc((void**) &d_reduce, maxReduce*REDUCE_MAX_BLOCKS*sizeof(QudaSumFloat)) 
...
void setBlock(int kernel, int length, QudaPrecision precision)
{
  ...
  // given the thread block size, if we can use a smaller grid than
  // the one requested, then use the smaller grid
  int blocks = min(blas_blocks[kernel][prec], max(length/blas_threads[kernel][prec], 1));
  blasBlock.x = blas_threads[kernel][prec];
  blasBlock.y = 1;
  blasBlock.z = 1;

  blasGrid.x = blas_blocks[kernel][prec];//blocks;
  blasGrid.y = 1;
  blasGrid.z = 1;
}
...
void copyCuda(cudaColorSpinorField &dst, const cudaColorSpinorField &src) {
  ...
  const int id = (src.Precision() == QUDA_DOUBLE_PRECISION || (dst.Precision() == QUDA_DOUBLE_PRECISION 
		  && src.Precision() == QUDA_SINGLE_PRECISION)) ? 0 : 1;
...
void axpbyCuda(const double &a, cudaColorSpinorField &x, const double &b, cudaColorSpinorField &y) {
  const int id = 2;
  setBlock(id, x.length, x.precision);
  checkSpinor(x, y);
...
void caxpyCuda(const Complex &a, cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  const int id = 8;
  setBlock(id, length, x.precision);
  blas_quda_bytes += 3*x.real_length*x.precision;
...
//
// double caxpyXmayCuda(float a, float *x, float *y, n){}
//
...
template <typename Float2>
__global__ void caxpyXmazKernel(Float2 a, Float2 *x, Float2 *y, Float2 *z, int len) {
...
__global__ void caxpyXmazHKernel(float2 a, short4 *xH, float *xN, short4 *yH, float *yN, 
				 int stride, int length) {
...
__global__ void caxpyXmazHKernel(float2 a, short2 *xH, float *xN, short2 *yH, float *yN, 
				 int stride, int length) {
...
void caxpyXmazCuda(const Complex &a, cudaColorSpinorField &x, cudaColorSpinorField &y, 
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    double2 a2 = make_double2(real(a), imag(a));
    caxpyXmazKernel<<<blasGrid, blasBlock>>>(a2, (double2*)x.v, (double2*)y.v, (double2*)z.v, 
					     length);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    float2 a2 = make_float2(real(a), imag(a));
    caxpyXmazKernel<<<blasGrid, blasBlock>>>(a2, (float2*)x.v, (float2*)y.v, (float2*)z.v, 
					     length);
  } else {
    if (x.siteSubset == QUDA_FULL_SITE_SUBSET) {
    ...
    if (x.nSpin == 4){ //wilson
      ...
      cudaBindTexture(0, texNorm3, z.norm, spinor_bytes/12);    
      float2 a2 = make_float2(real(a), imag(a));
      caxpyXmazHKernel<<<blasGrid, blasBlock>>>(a2, (short4*)x.v, (float*)x.norm, (short4*)y.v, 
						(float*)y.norm, y.stride, y.volume);
    } else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, spinor_bytes); 
      cudaBindTexture(0, texNorm1, x.norm, spinor_bytes/3);    
      ...
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/3);    
      float2 a2 = make_float2(real(a), imag(a));
      caxpyXmazHKernel<<<blasGrid, blasBlock>>>(a2, (short2*)x.v, (float*)x.norm, (short2*)y.v, 
						(float*)y.norm, y.stride, y.volume);
    }else{
      errorQuda("ERROR: nSpin=%d is not supported\n", x.nSpin);     
    }
...
__global__ void caxpyXmazDDHKernel(double3 *a_d, short4 *xH, float *xN, short4 *yH, float *yN, 
				 int stride, int length) {
  
  float2 a = make_float2(a_d->x / a_d->z, a_d->y / a_d->z);
...
__global__ void caxpyXmazDDHKernel(double3 *a_d, short2 *xH, float *xN, short2 *yH, float *yN, 
				   int stride, int length) {
...
void caxpyXmazDDCuda(cudaColorSpinorField &x, cudaColorSpinorField &y, cudaColorSpinorField &z) {
  ...
  blas_quda_flops += 8*x.real_length;
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    caxpyXmazDDKernel<<<blasGrid, blasBlock>>>(dd_reduce, (double2*)x.v, (double2*)y.v, (double2*)z.v, length);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    ...
    if (x.nSpin == 4){ //wilson
      ...
      cudaBindTexture(0, texHalf3, z.v, spinor_bytes); 
      cudaBindTexture(0, texNorm3, z.norm, spinor_bytes/12);    
      caxpyXmazDDHKernel<<<blasGrid, blasBlock>>>(dd_reduce, (short4*)x.v, (float*)x.norm, (short4*)y.v, 
						  (float*)y.norm, y.stride, y.volume);
    } else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, spinor_bytes); 
      cudaBindTexture(0, texNorm1, x.norm, spinor_bytes/3);    
      cudaBindTexture(0, texHalfSt2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/3);    
      caxpyXmazDDHKernel<<<blasGrid, blasBlock>>>(dd_reduce, (short2*)x.v, (float*)x.norm, (short2*)y.v, 
						  (float*)y.norm, y.stride, y.volume);
    }else{
      errorQuda("ERROR: nSpin=%d is not supported\n", x.nSpin);     
    }
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) sumD##suffix
#define REDUCE_TYPES FloatN *a
...
#define REDUCE_TYPES FloatN *aN, int stride
...
double normCuda(const cudaColorSpinorField &a) {
  ...
  if (a.precision == QUDA_DOUBLE_PRECISION) {
    return normDCuda<double>((double*)a.v, a.length, id, a.precision);
  } else if (a.precision == QUDA_SINGLE_PRECISION) {
    return normSCuda<double>((float2*)a.v, a.length/2, id, a.precision);
  } else {
	...
	return normHCuda<double>((float*)a.norm, a.stride, a.volume, id, a.precision);
    }else if (a.nSpin == 1) { //staggered
        cudaBindTexture(0, texHalfSt1, a.v, spinor_bytes);
        return normHStCuda<double>((float*)a.norm, a.stride, a.volume, id, a.precision);	
    }else{
...
#define REDUCE_TYPES FloatN *a, FloatN *b
...
#define REDUCE_TYPES FloatN *aN, FloatN *bN, int stride
...
double reDotProductCuda(cudaColorSpinorField &a, cudaColorSpinorField &b) {
  ...
  if (a.precision == QUDA_DOUBLE_PRECISION) {
    return reDotProductDCuda<double>((double*)a.v, (double*)b.v, a.length, id, a.precision);
  } else if (a.precision == QUDA_SINGLE_PRECISION) {
    return reDotProductSCuda<double>((float2*)a.v, (float2*)b.v, a.length/2, id, a.precision);
  } else {
    ...
    if (a.nSpin == 4){ //wilson
      ...
      return reDotProductHCuda<double>((float*)a.norm, (float*)b.norm, a.stride, a.volume, id, a.precision);
    }else if (a.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, a.v, spinor_bytes); 
      ...
      return reDotProductHStCuda<double>((float*)a.norm, (float*)b.norm, a.stride, a.volume, id, a.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, a.nSpin);      
      return 0;
    }
...
#define REDUCE_TYPES FloatN a, FloatN *x, FloatN *y
...
#define REDUCE_TYPES FloatN a, short4 *yH, float *yN, int stride
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) axpyNormH##suffix
#define REDUCE_TYPES FloatN a, short2 *yH, float *yN, int stride
#define REDUCE_PARAMS a, yH, yN, stride
#define REDUCE_AUXILIARY(i)						\
  RECONSTRUCT_HALF_SPINOR_ST(x, texHalfSt1, texNorm1, stride);		\
  RECONSTRUCT_HALF_SPINOR_ST(y, texHalfSt2, texNorm2, stride);		\
...
double axpyNormCuda(const double &a, cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  blas_quda_bytes += 3*x.real_length*x.precision;
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    return axpyNormFCuda<double>(a, (double*)x.v, (double*)y.v, x.length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    return axpyNormFCuda<double>((float)a, (float*)x.v, (float*)y.v, x.length, id, x.precision);
  } else {
    ...
    if (x.nSpin == 4){ //wilson
      ...
      return axpyNormHCuda<double>((float)a, (short4*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, x.bytes); 
      cudaBindTexture(0, texHalfSt2, y.v, x.bytes); 
      return axpyNormHCuda<double>((float)a, (short2*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
...
#define REDUCE_TYPES FloatN *x, FloatN *y
...
#define REDUCE_TYPES FloatN *d1, FloatN *d2, short4 *yH, float *yN, int stride
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) xmyNormH##suffix
#define REDUCE_TYPES FloatN *d1, FloatN *d2, short2 *yH, float *yN, int stride
#define REDUCE_PARAMS d1, d2, yH, yN, stride
#define REDUCE_AUXILIARY(i)						\
  RECONSTRUCT_HALF_SPINOR_ST(x, texHalfSt1, texNorm1, stride);		\
  RECONSTRUCT_HALF_SPINOR_ST(y, texHalfSt2, texNorm2, stride);		\
...
double xmyNormCuda(cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  blas_quda_bytes += 3*x.real_length*x.precision;
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    return xmyNormFCuda<double>((double*)x.v, (double*)y.v, x.length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    return xmyNormFCuda<double>((float*)x.v, (float*)y.v, x.length, id, x.precision);
  } else { 
    cudaBindTexture(0, texNorm1, x.norm, x.bytes/(x.nColor*x.nSpin));    
    cudaBindTexture(0, texNorm2, y.norm, x.bytes/(x.nColor*x.nSpin));    
    blas_quda_bytes += 3*x.volume*sizeof(float);
    ...
    if (x.nSpin ==4 ){ //wilsin
      cudaBindTexture(0, texHalf1, x.v, x.bytes); 
      cudaBindTexture(0, texHalf2, y.v, x.bytes); 
      return xmyNormHCuda<double>((char*)0, (char*)0, (short4*)y.v, (float*)y.norm, y.stride, y.volume, id, x.precision);
    }else if (x.nSpin == 1){
      cudaBindTexture(0, texHalfSt1, x.v, x.bytes); 
      cudaBindTexture(0, texHalfSt2, y.v, x.bytes); 
      return xmyNormHCuda<double>((char*)0, (char*)0, (short2*)y.v, (float*)y.norm, y.stride, y.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);                  
    }
  }
...
double caxpyNormCuda(const Complex &a, cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    double2 a2 = make_double2(real(a), imag(a));
    return caxpyNormFCuda<double>(a2, (double2*)x.v, (double2*)y.v, x.length/2, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    float2 a2 = make_float2(real(a), imag(a));
    return caxpyNormFCuda<double>(a2, (float2*)x.v, (float2*)y.v, x.length/2, id, x.precision);
  } else {
    cudaBindTexture(0, texNorm1, x.norm, x.bytes/(x.nColor*x.nSpin));    
    cudaBindTexture(0, texNorm2, y.norm, x.bytes/(x.nColor*x.nSpin));    
    blas_quda_bytes += 3*x.volume*sizeof(float);
    ...
    if (x.nSpin == 4){ //wilson
      ...
      cudaBindTexture(0, texHalf2, y.v, x.bytes); 
      float2 a2 = make_float2(real(a), imag(a));
      return caxpyNormHCuda<double>(a2, (short4*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, x.bytes); 
      cudaBindTexture(0, texHalfSt2, y.v, x.bytes); 
      float2 a2 = make_float2(real(a), imag(a));
      return caxpyNormHCuda<double>(a2, (short2*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
      return 0;
    }
...
#define REDUCE_TYPES FloatN a, FloatN *x, FloatN *y, FloatN *z
...
#define REDUCE_TYPES FloatN a, short4 *xH, float *xN, short4 *yH, float *yN, int stride
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) caxpyXmazNormXH##suffix
#define REDUCE_TYPES FloatN a, short2 *xH, float *xN, short2 *yH, float *yN, int stride
#define REDUCE_PARAMS a, xH, xN, yH, yN, stride
#define REDUCE_AUXILIARY(i)						\
  RECONSTRUCT_HALF_SPINOR_ST(x, texHalfSt1, texNorm1, stride);		\
  RECONSTRUCT_HALF_SPINOR_ST(y, texHalfSt2, texNorm2, stride);		\
...
double caxpyXmazNormXCuda(const Complex &a, cudaColorSpinorField &x, 
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    double2 a2 = make_double2(real(a), imag(a));
    return caxpyXmazNormXFCuda<double>(a2, (double2*)x.v, (double2*)y.v, (double2*)z.v, x.length/2, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    float2 a2 = make_float2(real(a), imag(a));
    return caxpyXmazNormXFCuda<double>(a2, (float2*)x.v, (float2*)y.v, (float2*)z.v, x.length/2, id, x.precision);
  } else {
    cudaBindTexture(0, texNorm1, x.norm, x.bytes/(x.nColor*x.nSpin));    
    cudaBindTexture(0, texNorm2, y.norm, x.bytes/(x.nColor*x.nSpin));    
    ...
    if (x.nSpin == 4){ //wilson
      ...
      float2 a2 = make_float2(real(a), imag(a));
      return caxpyXmazNormXHCuda<double>(a2, (short4*)x.v, (float*)x.norm, (short4*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, x.bytes); 
      cudaBindTexture(0, texHalfSt2, y.v, x.bytes); 
      cudaBindTexture(0, texHalfSt3, z.v, z.bytes); 
      float2 a2 = make_float2(real(a), imag(a));
      return caxpyXmazNormXHCuda<double>(a2, (short2*)x.v, (float*)x.norm, (short2*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
      return 0;
    }
...
#define REDUCE_TYPES FloatN a, FloatN b, FloatN *x, FloatN *y
...
#define REDUCE_TYPES FloatN a, FloatN b, short4 *xH, float *xN, short4 *yH, float *yN, int stride
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) cabxpyAxNormH##suffix
#define REDUCE_TYPES FloatN a, FloatN b, short2 *xH, float *xN, short2 *yH, float *yN, int stride
#define REDUCE_PARAMS a, b, xH, xN, yH, yN, stride
#define REDUCE_AUXILIARY(i)						\
  RECONSTRUCT_HALF_SPINOR_ST(x, texHalfSt1, texNorm1, stride);		\
  RECONSTRUCT_HALF_SPINOR_ST(y, texHalfSt2, texNorm2, stride);		\
...
double cabxpyAxNormCuda(const double &a, const Complex &b, cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    ...
    double2 b2 = make_double2(real(b), imag(b));
    return cabxpyAxNormFCuda<double>(a2, b2, (double2*)x.v, (double2*)y.v, x.length/2, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    float2 a2 = make_float2(a, 0);
    float2 b2 = make_float2(real(b), imag(b));
    return cabxpyAxNormFCuda<double>(a2, b2, (float2*)x.v, (float2*)y.v, x.length/2, id, x.precision);
  } else {
    cudaBindTexture(0, texNorm1, x.norm, x.bytes/(x.nColor*x.nSpin));    
    cudaBindTexture(0, texNorm2, y.norm, x.bytes/(x.nColor*x.nSpin));    
    blas_quda_bytes += 3*x.volume*sizeof(float);
    ...
    if (x.nSpin == 4){ //wilson
      ...
      float2 a2 = make_float2(a, 0);
      float2 b2 = make_float2(real(b), imag(b));
      return cabxpyAxNormHCuda<double>(a2, b2, (short4*)x.v, (float*)x.norm, (short4*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, x.bytes); 
      cudaBindTexture(0, texHalfSt2, y.v, x.bytes); 
      float2 a2 = make_float2(a, 0);
      float2 b2 = make_float2(real(b), imag(b));
      return cabxpyAxNormHCuda<double>(a2, b2, (short2*)x.v, (float*)x.norm, (short2*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
      return 0;
    }
...
#define REDUCE_TYPES FloatN *x, FloatN *y
#define REDUCE_PARAMS x, y
#define REDUCE_REAL_AUXILIARY(i) FloatN a = READ_DOUBLE2_TEXTURE(x, i);
#define REDUCE_IMAG_AUXILIARY(i) FloatN b = READ_DOUBLE2_TEXTURE(y, i);
...
#define REDUCE_TYPES FloatN *x, FloatN *y
#define REDUCE_PARAMS x, y
#define REDUCE_REAL_AUXILIARY(i) FloatN a = read_Float2(x, i);
#define REDUCE_IMAG_AUXILIARY(i) FloatN b = read_Float2(y, i);
#define REDUCE_REAL_OPERATION(i) (a.x*b.x + a.y*b.y)
#define REDUCE_IMAG_OPERATION(i) (a.x*b.y - a.y*b.x)
#include "reduce.h"
#undef REDUCE_FUNC_NAME
...
Complex cDotProductCuda(cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    ...
    cudaBindTexture(0, xTexDouble2, x.v, spinor_bytes); 
    cudaBindTexture(0, yTexDouble2, y.v, spinor_bytes); 
    dot = cDotProductDCuda<double2>((double2*)x.v, (double2*)y.v, length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    ...
    dot = cDotProductSCuda<double2>((float2*)x.v, (float2*)y.v, length, id, x.precision);
  } else {
    int spinor_bytes = x.length*sizeof(short);
    blas_quda_bytes += 2*x.volume*sizeof(float);
    if (x.nSpin == 4){
      ...
      cudaBindTexture(0, texHalf2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/12);    
      dot = cDotProductHCuda<double2>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    } else if (x.nSpin == 1){
      cudaBindTexture(0, texHalfSt1, x.v, spinor_bytes); 
      cudaBindTexture(0, texNorm1, x.norm, spinor_bytes/3);    
      cudaBindTexture(0, texHalfSt2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/3);    
      dot = cDotProductHStCuda<double2>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);      
    } 
  }
...
#define REDUCE_TYPES FloatN *x, FloatN a, FloatN *y, FloatN *z
#define REDUCE_PARAMS x, a, y, z
#define REDUCE_REAL_AUXILIARY(i)		\
  FloatN X = READ_DOUBLE2_TEXTURE(x, i);	\
  FloatN Y = READ_DOUBLE2_TEXTURE(y, i);	\
  FloatN Z = READ_DOUBLE2_TEXTURE(z, i);
...
Complex xpaycDotzyCuda(cudaColorSpinorField &x, const double &a, cudaColorSpinorField &y, cudaColorSpinorField &z) {
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    ...
    cudaBindTexture(0, yTexDouble2, y.v, spinor_bytes); 
    cudaBindTexture(0, zTexDouble2, z.v, spinor_bytes); 
    dot = xpaycDotzyDCuda<double2>((double2*)x.v, make_double2(a, 0), (double2*)y.v, (double2*)z.v, length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    dot = xpaycDotzySCuda<double2>((float2*)x.v, make_float2(a, 0), (float2*)y.v, (float2*)z.v, length, id, x.precision);
  } else {
    int spinor_bytes = x.length*sizeof(short);
    blas_quda_bytes += 4*x.volume*sizeof(float);
    if (x.nSpin ==4 ){//wilson
      ...
      cudaBindTexture(0, texHalf3, z.v, spinor_bytes); 
      cudaBindTexture(0, texNorm3, z.norm, spinor_bytes/12);    
      dot = xpaycDotzyHCuda<double2>(make_float2(a, 0), (short4*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
      ...
      cudaBindTexture(0, texHalfSt3, z.v, spinor_bytes); 
      cudaBindTexture(0, texNorm3, z.norm, spinor_bytes/3);    
      dot = xpaycDotzyHCuda<double2>(make_float2(a,0), (short2*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
    }
  }
...
Complex caxpyDotzyCuda(const Complex &a, cudaColorSpinorField &x, cudaColorSpinorField &y,
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    double2 a2 = make_double2(real(a), imag(a));
    dot = caxpyDotzyFCuda<double2>(a2, (double2*)x.v, (double2*)y.v, (double2*)z.v, x.length/2, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    float2 a2 = make_float2(real(a), imag(a));
    dot = caxpyDotzyFCuda<double2>(a2, (float2*)x.v, (float2*)y.v, (float2*)z.v, x.length/2, id, x.precision);
  } else {
    cudaBindTexture(0, texNorm1, x.norm, x.bytes/(x.nColor*x.nSpin));    
    cudaBindTexture(0, texNorm2, y.norm, x.bytes/(x.nColor*x.nSpin));    
    ...
    if (x.nSpin == 4){ //wilson
      ...
      dot = caxpyDotzyHCuda<double2>(a2, (short4*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, x.bytes); 
      cudaBindTexture(0, texHalfSt2, y.v, x.bytes); 
      cudaBindTexture(0, texHalfSt3, z.v, x.bytes); 
      dot = caxpyDotzyHCuda<double2>(a2, (short2*)y.v, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
    }
  }
...
#define REDUCE_TYPES FloatN *x, FloatN *y
#define REDUCE_PARAMS x, y
#define REDUCE_X_AUXILIARY(i) FloatN a = READ_DOUBLE2_TEXTURE(x, i);
#define REDUCE_Y_AUXILIARY(i) FloatN b = READ_DOUBLE2_TEXTURE(y, i);
...
#define REDUCE_TYPES FloatN *xN, FloatN *yN, int stride
...
double3 cDotProductNormACuda(cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    ...
    cudaBindTexture(0, xTexDouble2, x.v, spinor_bytes); 
    cudaBindTexture(0, yTexDouble2, y.v, spinor_bytes); 
    return cDotProductNormADCuda<double3>((double2*)x.v, (double2*)y.v, length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    return cDotProductNormASCuda<double3>((float2*)x.v, (float2*)y.v, length, id, x.precision);
  } else {
    int spinor_bytes = x.length*sizeof(short);
    blas_quda_bytes += 2*x.volume*sizeof(float);
    if (x.nSpin == 4){ //wilson
      ...
      cudaBindTexture(0, texHalf2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/12);    
      return cDotProductNormAHCuda<double3>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    } else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, spinor_bytes); 
      cudaBindTexture(0, texNorm1, x.norm, spinor_bytes/3);    
      cudaBindTexture(0, texHalfSt2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/3);    
      return cDotProductNormAHStCuda<double3>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
    }
  }
...
double3 cDotProductNormBCuda(cudaColorSpinorField &x, cudaColorSpinorField &y) {
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    ...
    cudaBindTexture(0, xTexDouble2, x.v, spinor_bytes); 
    cudaBindTexture(0, yTexDouble2, y.v, spinor_bytes); 
    return cDotProductNormBDCuda<double3>((double2*)x.v, (double2*)y.v, length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    return cDotProductNormBSCuda<double3>((float2*)x.v, (float2*)y.v, length, id, x.precision);
  } else {
    int spinor_bytes = x.length*sizeof(short);
    blas_quda_bytes += 2*x.volume*sizeof(float);
    if (x.nSpin == 4){ //wilson
      ...
      cudaBindTexture(0, texHalf2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/12);    
      return cDotProductNormBHCuda<double3>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    } else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, spinor_bytes); 
      cudaBindTexture(0, texNorm1, x.norm, spinor_bytes/3);    
      cudaBindTexture(0, texHalfSt2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/3);    
      return cDotProductNormBHStCuda<double3>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
    }
  }
...
#define REDUCE_TYPES FloatN a, FloatN *x, FloatN b, FloatN *y, FloatN *z, FloatN *w, FloatN *u
#define REDUCE_PARAMS a, x, b, y, z, w, u
#define REDUCE_X_AUXILIARY(i)				\
  FloatN X = READ_DOUBLE2_TEXTURE(x, i);		\
  FloatN Y = READ_DOUBLE2_TEXTURE(y, i);		\
  FloatN W = READ_DOUBLE2_TEXTURE(w, i);				
#define REDUCE_Y_AUXILIARY(i)			\
  FloatN Z = read_Float2(z, i);			\
  Z.x += a.x*X.x - a.y*X.y;			\
  Z.y += a.y*X.x + a.x*X.y;			\
  Z.x += b.x*Y.x - b.y*Y.y;			\
  Z.y += b.y*Y.x + b.x*Y.y;			\
...
#define REDUCE_PARAMS a, x, b, y, z, w, u
#define REDUCE_X_AUXILIARY(i)				\
  FloatN X = read_Float2(x, i);		\
  FloatN Y = read_Float2(y, i);		\
  FloatN W = read_Float2(w, i);		
#define REDUCE_Y_AUXILIARY(i)			\
  FloatN Z = read_Float2(z, i);	\
  Z.x += a.x*X.x - a.y*X.y;			\
  Z.y += a.y*X.x + a.x*X.y;			\
...
#define REDUCE_TYPES FloatN a, FloatN b, short4 *yH, float *yN, short4 *zH, float *zN, float *wN, float *uN, int stride
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) caxpbypzYmbwcDotProductUYNormYH##suffix
#define REDUCE_TYPES FloatN a, FloatN b, short2 *yH, float *yN, short2 *zH, float *zN, float *wN, float *uN, int stride
#define REDUCE_PARAMS a, b, yH, yN, zH, zN, wN, uN, stride
#define REDUCE_X_AUXILIARY(i)						\
  RECONSTRUCT_HALF_SPINOR_ST(x, texHalfSt1, texNorm1, stride);		\
  RECONSTRUCT_HALF_SPINOR_ST(y, texHalfSt2, texNorm2, stride);		\
...
#undef HOST_RESULT_REDUCE
#undef REDUCE_BUF
#define REDUCE_BUF dd_reduce

// This is a desperate hack to get sync free reductions for the Schwarz solver

//
// void cDotProductNormALocalCuda(float2 *a, float2 *b, int n) {}
//
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) cDotProductNormALocalD##suffix
#define REDUCE_TYPES FloatN *x, FloatN *y
#define REDUCE_PARAMS x, y
#define REDUCE_X_AUXILIARY(i) FloatN a = READ_DOUBLE2_TEXTURE(x, i);
#define REDUCE_Y_AUXILIARY(i) FloatN b = READ_DOUBLE2_TEXTURE(y, i);
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) cDotProductNormALocalS##suffix
#define REDUCE_TYPES FloatN *a, FloatN *b
#define REDUCE_PARAMS a, b
#define REDUCE_X_AUXILIARY(i)
#define REDUCE_Y_AUXILIARY(i)
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) cDotProductNormALocalH##suffix
#define REDUCE_TYPES FloatN *xN, FloatN *yN, int stride
#define REDUCE_PARAMS xN, yN, stride
#define REDUCE_X_AUXILIARY(i)						\
  READ_HALF_SPINOR(x, texHalf1, stride);				\
...
template <unsigned int reduce_threads, typename FloatN>
#define REDUCE_FUNC_NAME(suffix) cDotProductNormALocalHSt##suffix
#define REDUCE_TYPES FloatN *xN, FloatN *yN, int stride
#define REDUCE_PARAMS xN, yN, stride
#define REDUCE_X_AUXILIARY(i)						\
  READ_HALF_SPINOR_ST(x, texHalfSt1, stride);				\
...
double3 cDotProductNormALocalCuda(cudaColorSpinorField &x, cudaColorSpinorField &y) {
  if (x.siteSubset == QUDA_FULL_SITE_SUBSET) 
    return cDotProductNormALocalCuda(x.Even(), y.Even()) + cDotProductNormALocalCuda(x.Odd(), y.Odd());
  ...
  if (x.precision == QUDA_DOUBLE_PRECISION) {
    ...
    cudaBindTexture(0, xTexDouble2, x.v, spinor_bytes); 
    cudaBindTexture(0, yTexDouble2, y.v, spinor_bytes); 
    return cDotProductNormALocalDCuda<double3>((double2*)x.v, (double2*)y.v, length, id, x.precision);
  } else if (x.precision == QUDA_SINGLE_PRECISION) {
    return cDotProductNormALocalSCuda<double3>((float2*)x.v, (float2*)y.v, length, id, x.precision);
  } else {
    int spinor_bytes = x.length*sizeof(short);
    blas_quda_bytes += 2*x.volume*sizeof(float);
    if (x.nSpin == 4){ //wilson
      ...
      cudaBindTexture(0, texHalf2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/12);    
      return cDotProductNormALocalHCuda<double3>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    } else if (x.nSpin == 1){ //staggered
      cudaBindTexture(0, texHalfSt1, x.v, spinor_bytes); 
      cudaBindTexture(0, texNorm1, x.norm, spinor_bytes/3);    
      cudaBindTexture(0, texHalfSt2, y.v, spinor_bytes); 
      cudaBindTexture(0, texNorm2, y.norm, spinor_bytes/3);    
      return cDotProductNormALocalHStCuda<double3>((float*)x.norm, (float*)y.norm, x.stride, x.volume, id, x.precision);
    }else{
      errorQuda("%s: nSpin(%d) is not supported\n", __FUNCTION__, x.nSpin);            
    }
  }
