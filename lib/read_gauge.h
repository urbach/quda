...
#define READ_GAUGE_MATRIX_8_SHORT4_TEX(G, gauge, dir, idx, stride)	\
  ...
  float4 G##3 = make_float4(0,0,0,0);					\
  float4 G##4 = make_float4(0,0,0,0);					\
  (G##3).z = (G##0).x = dp.pi_f*(G##0).x;						\
  (G##3).w = (G##0).y = dp.pi_f*(G##0).y;
...
#ifndef MULTI_GPU
#define do_boundary (ga_idx >= lp.X4X3X2X1hmX3X2X1h)
#else
#define do_boundary ( (dp.Pt0 && (ga_idx >= lp.Vh)) || ( dp.PtNm1 && (ga_idx >= lp.X4X3X2X1hmX3X2X1h) && (ga_idx < lp.Vh) ) )
#endif
...
#define RECONSTRUCT_MATRIX_12_DOUBLE(dir)				\
  ...
  double u0 = (dir < 6 ? dp.anisotropy : (do_boundary ? dp.t_boundary : 1)); \
...
#define RECONSTRUCT_MATRIX_12_SINGLE(dir)			\
  ...
  ACC_CONJ_PROD(g22, +g00, +g11);				\
  ACC_CONJ_PROD(g22, -g01, +g10);				\
  float u0 = (dir < 6 ? dp.anisotropy_f : (do_boundary ? dp.t_boundary_f : 1)); \
...
#define RECONSTRUCT_MATRIX_8_SINGLE(dir)				\
  ...
  float2 u0_2 = (dir < 6 ? dp.An2 : (do_boundary ? dp.TB2 : dp.No2)); \
