...
#elif (DD_PREC == 1) 
#define DD_PARAM5 const int oddBit, const float2 *x, const float *xNorm, const float a
#else
...
#define DD_CONCAT_INTERNAL(n,r,d,x) n ## r ## d ## x ## InternalKernel
#define DD_FUNC_INTERNAL(n,r,d,x)  DD_CONCAT_INTERNAL(n,r,d,x)
__global__ void	DD_FUNC_INTERNAL(DD_FNAME, DD_RECON_F, DD_DAG_F, DD_XPAY_F)
     (DD_PARAM1, DD_PARAM2,  DD_PARAM4, DD_PARAM5) {
#define INTERNAL_COMPUTE
 // #if (DD_PREC == 1)
#if 1
  #include "dslash_staggered_core.h"
#endif
#undef INTERNAL_COMPUTE
}

#define DD_CONCAT_BOUNDARY(n,r,d,x) n ## r ## d ## x ## BoundaryKernel
#define DD_FUNC_BOUNDARY(n,r,d,x)  DD_CONCAT_BOUNDARY(n,r,d,x)
__global__ void	DD_FUNC_BOUNDARY(DD_FNAME, DD_RECON_F, DD_DAG_F, DD_XPAY_F)
     (DD_PARAM1, DD_PARAM2,  DD_PARAM4, DD_PARAM5) {
#define BOUNDARY_COMPUTE
 // #if (DD_PREC == 1)
#if 1 
  #include "dslash_staggered_core.h"
#endif
#undef BOUNDARY_COMPUTE
}
...
#undef DD_FNAME
#undef DD_CONCAT
#undef DD_CONCAT_INTERNAL
#undef DD_CONCAT_BOUNDARY
#undef DD_FUNC
#undef DD_FUNC_INTERNAL
#undef DD_FUNC_BOUNDARY
