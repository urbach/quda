		      ...
		      float2* momEven, float2* momOdd)
{
    ...
    */
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(mymu, ad_link_nbr_idx, ad_link_sign, linka);	
    if (mu_positive){
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(mymu, bc_link_nbr_idx, bc_link_sign, linka);
    if (mu_positive){    
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(mysig, ab_link_nbr_idx, ab_link_sign, linkb);
    if (sig_positive){        
		...
		float2* momEven, float2* momOdd)
{
    ...
    */
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(mymu, ad_link_nbr_idx, ad_link_sign, linkc);	
    if (mu_positive){
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(mysig, ab_link_nbr_idx, ab_link_sign, linka);
    if (sig_positive){        
			  ...
			  float2* momEven, float2* momOdd)
{
	...
	RECONSTRUCT_MATRIX_12_SINGLE(OPP_DIR(mu), new_idx, sign, linka);		
	...
	RECONSTRUCT_MATRIX_12_SINGLE(OPP_DIR(mu), sid, sign, linka);	
	...
	RECONSTRUCT_MATRIX_12_SINGLE(mu, sid, sign, linka);
	MAT_MUL_HW(linka, hwa, hwb);
