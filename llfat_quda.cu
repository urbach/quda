				 ...
				 float mycoeff)
{
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(nu, mem_idx, sign, a);
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(mu, new_mem_idx, sign, b);
    ...
    RECONSTRUCT_MATRIX_12_SINGLE(nu, new_mem_idx, sign, c);
    ...
    COMPUTE_RECONSTRUCT_SIGN(sign, nu, new_x1, new_x2, new_x3, new_x4);        
    RECONSTRUCT_MATRIX_12_SINGLE(nu, (new_mem_idx), sign, a);
		     ...
		     float2* fatlink_even, float2* fatlink_odd)
{
	...
	RECONSTRUCT_MATRIX_12_SINGLE(dir, mem_idx, sign, tempa);
...
#define UNBIND_ALL_TEXTURE do{			\
	...
	cudaUnbindTexture(fatLink0TexSingle);	\
	cudaUnbindTexture(fatLink1TexSingle);	\
...
#define BIND_SITE_AND_FAT_LINK_REVERSE do {				\
	...
	cudaBindTexture(0, fatLink1TexSingle, cudaFatLink.even, cudaFatLink.bytes); \
	cudaBindTexture(0, fatLink0TexSingle, cudaFatLink.odd,  cudaFatLink.bytes); \
    }while(0)
